...
| | `test_skill_graph_semantics.py` | Layer-1/2 semantics: fallback chains, sealing, atomicity, schema |
...
| | `test_higher_layer_graphs.py` | Gold graphs: validity, grounding, coarse/fine equivalence, committed JSON and SVG |
| | `test_granularity_evaluation.py` | Stage-5 sweep: agreement metrics, rejection tally, the scripted dry run |
| | `test_experiment_figures.py` | The committed figures regenerate from the committed data |
| `planning/` `mshab/experiments/planning` | `test_planning_boundary.py` | Documents, scripted proposer, assembler, validator rejections |
| | `test_task_controller.py` | Symbolic environment and the `execute -> observe -> re-decide` controller on the scenarios |
| | `test_deepseek_proposer.py` | The real model behind a fake transport: prompts, parsing, retry rounds |
| `rollout/` `mshab/experiments/rollout` | `test_rollout.py` | Stage 6 on CPU: episode from an official plan, facts, node → subtask, rule proposer, dry run |
